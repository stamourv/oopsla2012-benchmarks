...
(define (kernel-decode base-pixel delta-pixel)
  (let ([clampable (- delta-pixel
                      (negate-pixel (reduce-precision base-pixel)))])
    (bitwise-and #xffffff
                 (arithmetic-shift (clamp-pixel clampable)
                                   2))))
...
(define frames0
  (map (lambda (f) (cons (car f) (jpeg-bytes->bitmap (cdr f))))
       (with-input-from-file "captured-frames" read)))
